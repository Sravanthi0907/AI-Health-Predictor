...
def home():
    if request.method == 'POST':
        # Check if speech-to-text data is available
        speech_text = request.form.get('speech_text')
        symptoms = request.form.get('symptoms')

        # Combine speech input and text input if speech is detected
        if speech_text and speech_text.strip():
            symptoms = speech_text.strip()
            print(f"Speech input detected: {symptoms}")
        else:
            print(f"Text input detected: {symptoms}")
        ...
        if not symptoms or symptoms == "Symptoms":
            ...
            # Get the predicted disease and relevant details
